...
jQuery( function ( $ ) {
	...
	const AWOOC = {
		...
		popup: function ( e ) {
			...
			$.blockUI( {
					message:          awooc_scripts_settings.popup.template,
	...
	/*		.on( 'click', '.awooc-custom-order-button', function( event ) {
	 ...
	 ? ''
	 : '\n' + awooc_scripts_translate.title + data.title,
	 ...
	 ? ''
	 : '\n' + awooc_scripts_translate.price + data.pricenumber,
	 outQty: data.qty === false
	 ? ''
	 : '\n' + awooc_scripts_translate.product_qty + productQty,
	 outSum: data.sum === false
	 ? ''
	 : '\n' + awooc_scripts_translate.sum + data.sumnumber
	 };
	 ...
	 // Загружаем форму.
	 $(  data.popup ).find( '.awooc-col.columns-right' ).html( data.form );
